    ...
    <div class="funds-main">
      <div class="funds-aside">
        <div class="funds-aside-list">
          <router-link to="/funds/funds_manage" class="asideActive">
            <i class="icon-account"></i>{{Internationalization.funds_manage}}</router-link>
          <router-link to="/funds/finance_manage">
            <i class="icon-trade "></i>{{Internationalization.finance_manage}}</router-link>
          <router-link to="/funds/address_manage">
            <i class="icon-address"></i>{{Internationalization.address_manage}}</router-link>
        </div>
      </div>
      <div class="funds-content">
        ...
        <div class="fund-manage-top" v-if="coinlist">
          ...
          <div class="fund-manage-tool" v-if="accountTabClass !== 'contract'">
            <div class="fund-manage-allassets">{{Internationalization.total_money}}: {{totalBalance}}
              <span>{{totalBalanceSymbol}}</span>
              <span> ≈ {{CNY}}</span>
            </div>
              ...
              <table>
                ...
                <template v-if="accountTabClass === 'transaction'">
                  ...
                  <template v-for="(item, key) in accountList">
                    <tr v-if="item.checked">
                      <!--{{imgSrc}}-->
                      <td class="firstTd">
                        <img :src="coinlist[item.coinName].icon || imgSrc" />
                        <span>{{item.coinName}}</span><span>{{item.chineseName}}</span><span>{{item.fullName}}</span>
                      </td>
                      ...
                      <td>
                        <span class="opear">
                          <a class="deff-text" @click="rechargeShow(key,'recharge')" href="javascript:void(0)" v-if="item.depositOpen">
                      ...
                      <td colspan="6">
                        ...
                        <div v-else class="rechargeCon clearfix">
                          <ul>
                            ...
                            <li v-if="labelShow">
                              <input readonly="readonly" class="labelStr" ref="labelId" :value="labelStr" />
                              <a class="copyLabel" href="javascript:;" @click="copyLabel">{{Internationalization.copytag}}</a>
                            </li>
                            <li class="warn-txt">
                      ...
                      <td colspan="6">
                        <div class="withdrawCon clearfix">
                          <ul style="width: 100%;">
                            ...
                            <li>
                              <inline-input :property="formData.addressList" v-model="formData.addressList.value" v-on:onevents="somethings">
                              </inline-input>
                            </li>
                          ...
                          <ul>
                            <inline-input :property="formData.canWithdraw" v-model="formData.canWithdraw.value" v-on:onevents="somethings">
                            </inline-input>
                            <li class="feeNote">{{Internationalization.canWithdraw}}:{{item.normal_balance}}</li>
                          </ul>
                          <ul>
                            <inline-input :property="formData.feeWithdraw" v-model="formData.feeWithdraw.value" v-on:onevents="somethings">
                            </inline-input>
                ...
                <template v-if="accountTabClass === 'Outside'">
                  ...
                  <template v-for="(item,index) in accountList">
                    <tr v-if="item.checked">
                      <td class="firstTd">
                        <img :src="(otcCoinlist_arr && otcCoinlist_arr[index].icon) || imgSrc" />
                        <span>{{item.coinSymbol}}</span><span>{{item.chineseName}}</span><span>{{item.fullName}}</span></td>
                      <td>{{item.total_balance}}</td>
                      <td>{{item.normal}}</td>
                      <td>{{item.lock}}</td>
                      ...
                      <td>
                        <span class="opear">
                          <a class="deff-text" href="javascript:void(0)" @click="transferopera('in',item)">{{Internationalization.transferIn}}</a>
                        </span>
                        <span class="opear">
                          <a href="javascript:void(0)" @click="transferopera('out',item)">{{Internationalization.transferOut}}</a>
                        </span>
                      </td>
                    </tr>
                  </template>
        ...
        <!--弹窗-->
        <!-- 实名验证弹窗提示 -->
        <el-dialog :title='Internationalization.notice' :flag='goRealnameDialog' name='goRealnameDialog' :width='450' :dialogClose='closeDialog'>
        ...
        <el-dialog :title='Internationalization.notice' :flag='setSafetyDialog' name='setSafetyDialog' :width='450' :dialogClose='closeDialog'>
        ...
        <!--提现-->
        <el-dialog :title='Internationalization.confirm_withdraw' :flag='confirmWithdraw' name='confirmWithdraw' :width='450' :dialogClose='closeDialog'>
          <div slot="content">
            <ul class="infor-list">
              ...
              <li>{{Internationalization.actual}}:
                <s class="amount">{{amount}}</s> {{coinSymbol}}</li>
              <li>
                <div v-for="(item, key) in formList" :key='key'>
                  <inline-input v-bind:property="item" v-model="item.value" v-on:onevents="somethings">
                  </inline-input>
                </div>
              </li>
            </ul>
        ...
        <!--提现成功-->
        <el-dialog :title='Internationalization.wait_note' :flag='successWithdraw' name='successWithdraw' :width='450' :dialogClose='closeDialog'>
          <div slot="content">
            <ul class="infor-list">
              <li>{{Internationalization.applyTime}}: {{successData.applyTime}}</li>
              <li>{{Internationalization.symbol}}: {{successData.coinSymbol}}</li>
              <li>{{Internationalization.actual}}:
                <span class="amount">{{successData.amount}}</span>{{successData.coinSymbol}}</li>
              <li>{{Internationalization.feeWithdraw}}: {{feeWithdrawCon}}{{coinSymbol}}</li>
        ...
        <!--转出-->
        <el-dialog :title='Internationalization.transfer' :flag='transferout' name='transferout' :width='450' :dialogClose='closeDialog'>
          <div slot="content">
            <ul class="transfer-list transferOpe">
              <li>
                <span>{{Internationalization.from}}</span>
                <span class="current-acc">{{Internationalization.outside_acc}}</span>
              </li>
              <li>
                <span>{{Internationalization.into}}</span>
                <span class="current-acc">{{Internationalization.transaction_acc}}</span>
              </li>
              <li>
                <span>{{Internationalization.transferOutAmount}}</span>
                <input type="number" :placeholder="Internationalization.transferOutAmount"
              ...
              <li>
                ...
                <div>
                  <i>{{Internationalization.canTransferOutAmount}}:</i>
                  <i>{{normal}}</i>
                  <a href="javascript:void(0)" @click="alltransfer(transferItem.normal)">{{Internationalization.allTransferOut}}</a>
                </div>
              </li>
              <li class="error-note">{{errorInfo}}</li>
        ...
        <!--转入-->
        <el-dialog :title='Internationalization.transfer' :flag='transferin' name='transferin' :width='450' :dialogClose='closeDialog'>
          <div slot="content">
            <ul class="transfer-list transferOpe">
              <li>
                <span>{{Internationalization.from}}</span>
              ...
              <li>
                <span>{{Internationalization.transferInAmount}}</span>
                <input type="number" :placeholder="Internationalization.transferInAmount"
                  v-model="transferAmount" />
                <span>{{coinSymbol}}</span>
              </li>
              <li>
                <span class="usable-quantity"></span>
                <div>
                  <i>{{Internationalization.canTransferInAmount}}:</i>
                  <i>{{exchangeNormal}}</i>
                  <a href="javascript:void(0)" @click="alltransfer(transferItem.exchangeNormal)">{{Internationalization.allTransferIn}}</a>
                </div>
              </li>
              <li class="error-note">{{errorInfo}}</li>
            </ul>
      ...
      setAccountList () {
        ...
        if (this.accountTabClass === 'Outside') {
          for (let i in this.accountList) {
            ...
            this.accountList[i].total_balance = this._P.fixD(this.accountList[i].total_balance, this.otcCoinlist_arr[i]
              .showPrecision)
        ...
        setTimeout(function () {
          _this.axios({
            ...
            if (data.code === '0') {
              ...
              _this.$store.dispatch('setTipState', {
                text: data.msg,
                type: 'error'
              })
            }
      ...
      withdrawShow (item, operation) {
        ...
        if (item === 'XRP' || item === 'EOS') {
          ...
          for (let i in this.accountList[item].withdrawAddressMap) {
            ...
            let obj = {
              value: arr[0] + '(' + this.accountList[item].withdrawAddressMap[i].label + ')',
              code: this.accountList[item].withdrawAddressMap[i].id,
              numberId: arr[1]
            }
            addList.push(obj)
          }
          ...
          for (let i in this.accountList[item].withdrawAddressMap) {
            let obj = {
              value: this.accountList[item].withdrawAddressMap[i].address + '(' + this.accountList[item].withdrawAddressMap[i].label + ')',
              code: this.accountList[item].withdrawAddressMap[i].id
            }
            addList.push(obj)
          }
      ...
      submitWithdraw (coin) {
        ...
        for (let item in this.formData) {
          ...
          if (this.formData[item].name === 'canWithdraw') {
            ...
            if (this._P.fixD(parseFloat(canWithdraw) - parseFloat(feeWithdraw), this.coinlist[coin.coinName].showPrecision) <
              parseFloat(coin.withdraw_min)) {
