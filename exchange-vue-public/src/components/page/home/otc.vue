         ...
         <dl class="hot-box clearfix">
           <dt class="portrait" @click="getUid(item.userId)">
            ...
            <dd class="nickname" @click="getUid(item.userId)">
              {{item.otcNickName}}
              <span class="sellColor" v-if="item.side === 'SELL' && false">({{$t('otc.index.sell')}})</span>
              <span class="buyColor" v-if="item.side === 'BUY' && false">({{$t('otc.index.buy')}})</span>
            </dd>
           ...
           <dd class="turnov sellbg" v-if="item.side === 'SELL'" @click="busin(item.advertId, item.userId)">{{$t('otc.index.sell')}}{{item.coin}}</dd>
           <dd class="turnov buybg" v-if="item.side === 'BUY'" @click="busin(item.advertId, item.userId)">{{$t('otc.index.buy')}}{{item.coin}}</dd>
         </dl>
       </div>
     </div>
     <!--币对市场-->
     <div class="market front-color" v-if="false">
        <div class="header">
           <ul>
...
import adHall from '@/product/exchange-vue-otc/otc/ad_hall'
export default {
  name: 'otc_home',
  components: {
    slider,
