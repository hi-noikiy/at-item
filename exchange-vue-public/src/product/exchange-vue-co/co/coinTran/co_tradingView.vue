...
export default {
  ...
  watch: {
    // 监听切换货币对
    '$store.state.coBaseData._nowSymbol' (val, old) {
      // console.log(old)
      if (!val) return false
      this.symbolName = {
        base: val.split('/')[0].toUpperCase(),
        count: val.split('/')[1].toUpperCase(),
        name: val.toUpperCase(),
        symbol: val.split('/')[0] + val.split('/')[1]
      }
      // console.log(this.symbolName)
      this.isshowLoading = true
      if (this.symbolName.base && window.tvWidget) {
  ...
  methods: {
    init () {
      this.symbolName = {
        base: this.$store.state.coBaseData._nowSymbol.split('/')[0].toUpperCase(),
        count: this.$store.state.coBaseData._nowSymbol.split('/')[1].toUpperCase(),
        name: this.$store.state.coBaseData._nowSymbol.toUpperCase(),
        symbol: this.$store.state.coBaseData._nowSymbol.split('/')[0] + this.$store.state.coBaseData._nowSymbol.split('/')[1]
      }
    ...
    fixDepthNumber (symbolName) {
      let b = '1'
      let n = this.marketList[symbolName.count.toLowerCase()][symbolName.name.toLowerCase()].price
      for (let i = 0; i < n; i++) {
        b = b + '0'
      }
      return parseFloat(b)
