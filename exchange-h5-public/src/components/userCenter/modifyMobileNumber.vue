<template>
  <section id="com-modifyMobileNumber">
    <div class="areaSelect" v-if="selectFlag">
      <areaSelect :activeFn='activeFn' />
    </div>
    <h6 class="title">{{internationalization.editPhone}}</h6>
    <p>{{internationalization.oldPhone}}：{{userinfo.mobileNumber}}</p>
    <div>
      <comInput :inpPlaceholder='internationalization.smsAuthCode' iconUrl='icon-code' inpType='text' :errMess='phoneReg.codeOld' ref='codeOlds' v-model="phoneFrom.codeOld">
        <countDown @count='codeOldClick' ref='codeOld' />
      </comInput>
    </div>
    <p>{{internationalization.newBindPhone}}</p>
    <div>
      <comInput :inpReadOnly='true' :disabled='true' :inpPlaceholder='internationalization.area' iconUrl='icon-Country' :errMess='phoneReg.area' v-model="phoneFrom.area" ref='area'  :clickFn="_areaInp" />
    </div>
    <!-- 手机号 -->
    <div>
      <comInput :inpPlaceholder='internationalization.phoneNumber' iconUrl='icon-phone' inpType='text' ref='phone' :errMess='phoneReg.phone' v-model="phoneFrom.phone" />
    </div>
    <!--滑动验证-->
    <div>
      <AliyunCaptcha style="width:100%" scene="other" :appkey='$store.state.baseData.appKey' v-on:callback='onCaptcha'></AliyunCaptcha>
      <div class="alyError" v-if="alyErrorFlag"><span>{{internationalization.alyError}}</span></div>
    </div>
    <!-- 验证码 -->
    <div>
      <comInput :inpPlaceholder='internationalization.smsAuthCode' iconUrl='icon-code' inpType='text' :errMess='phoneReg.code' ref='codes' v-model="phoneFrom.code">
        <countDown @count='codeClick' ref='code' :timeEnd='timeEnd' />
      </comInput>
    </div>
    <div v-if="userinfo.googleStatus === 1">
      <comInput :inpPlaceholder='internationalization.googleCode' iconUrl='icon-code' inpType='text' :errMess='phoneReg.googleCode' ref='googleCode' v-model="phoneFrom.googleCode">
      </comInput>
    </div>
...
export default {
  ...
  mounted () {
    ...
    this.phoneReg.codeOld.error = this.internationalization.rightMobileCode
  ...
  watch: {
    ...
    'phoneFrom.codeOld' (newValue, oldValue) { this.setErrorFlag(newValue, 'codeOld') },
  ...
  methods: {
    ...
    submit () {
      let { area, phone, code, googleCode, codeOld } = this.phoneReg
      if (this.userinfo.googleStatus === 0) {
        googleCode.flag = true
      }
      if (area.flag && phone.flag && code.flag && googleCode.flag && codeOld.flag) {
        this.axios({
          url: this.$store.state.url.user.mobile_update,
          params: {
            ...
            'authenticationCode': this.phoneFrom.codeOld
          },
          method: 'post'
        }).then((data) => {
          if (data.code === '0') {
